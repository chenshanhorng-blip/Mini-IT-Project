...
func _ready():
	...
	# 加载死亡画面
	death_screen = load("res://scenes/ui/death_screen.tscn").instantiate()
...
# 在检查点复活
func respawn_at_checkpoint(position: Vector2):
	print("🔄 复活在: ", position)
...
# 复活信号回调
func _on_player_respawn(position: Vector2):
	respawn_at_checkpoint(position)
