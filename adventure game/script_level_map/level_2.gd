...
# (EN: The feedback panel is nested inside a teammate's Node2D/CanvasLayer wrapper,
# so the full path is needed to reach the actual node that has the script attached)
...
# Spawns Player 2 only once, called from _ready() — NOT inside the diamond loop above,
# to avoid the earlier bug where Player 2 spawned multiple times
